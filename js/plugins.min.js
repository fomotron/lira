...
function initWallet(){
	...
	window.tronWeb.setFullNode("https://api.trongrid.io");
	window.tronWeb.setEventServer("https://api.trongrid.io");
	window.tronWeb.setSolidityNode("https://api.trongrid.io");
	tronWeb.setHeader({ "TRON-PRO-API-KEY": 'd35618c0-9232-4389-a674-d4ac63fff94b' });
	
	tronWeb.setFullNode("https://api.trongrid.io");
	tronWeb.setEventServer("https://api.trongrid.io");
	tronWeb.setSolidityNode("https://api.trongrid.io");
	state.address = tronWeb.defaultAddress.base58;
	var my_address_link = state.tron_domain + "/#/address/" + state.address;
	if (!tronWeb.isAddress(state.address)) {
			my_address_link="#";
			state.address="未连接钱包";
			var r=confirm("未连接到钱包是否刷新!");
			if (r==true){
				initWallet();
				return;
			}
	...
	var contract_url = state.tron_domain + "/#/contract/" + contract_address;


	jQuery(".view_contract").attr("href", contract_url);
	navMenu();
...
function BuyToken() {
	...
	if (!tronWeb.isAddress(state.address)) {
		...
		var r=confirm("未连接到钱包请刷新页面在进行购买!");
		if (r==true){
			initWallet();
			return;
		}
...
function BindPage(output) {
    ...
    $("#last_time").html(new Date(tronWeb.toDecimal(output.last_buy_time) * 1000));

    var price = math.chain(tronWeb.toDecimal(output.price)).divide(math.pow(10, 6)).done();
